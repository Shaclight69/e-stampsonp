...
export const getEvent =
  ...
  async (dispatch) => {
    try {
      ...
      let link = `${FRONTEND_URI}/api/v3/events?keyword=${keyword}&page=${currentPage}`;

      if (category) {
        link = `${FRONTEND_URI}/api/v3/events?keyword=${keyword}&page=${currentPage}&category=${category}`;
      }
...
export const getAdminEvent = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${FRONTEND_URI}/api/v3/admin/events`);
...
export const createEvent = (eventData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `${FRONTEND_URI}/api/v3/admin/event/new`,
      eventData,
      config
    );
...
export const updateEvent = (id, eventData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `${FRONTEND_URI}/api/v3/admin/event/${id}`,
      eventData,
      config
    );
...
export const getEventDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${FRONTEND_URI}/api/v3/event/${id}`);
